defmodule BasicWeb.SphereLive do
  ...
  def mount(params, _session, socket) do
    ...
    pysical_content_root_folder =
      Path.join(Application.fetch_env!(:sphere, :local_root), "content")

    # TODO: "communitex"のようなデフォルトをconfigで指定可能に
    content =
      folder_in_between_if_exists(
        params["content"],
        pysical_content_root_folder,
        "communitex.org"
      )
    ...
    css_pysical_folder =
      Path.join([Application.fetch_env!(:sphere, :local_root), content_folder, "css"])
  ...
  def render(assigns) do
    ...
    try do
      ...
      if String.contains?(content_path, [".html", ".md"]),
        do: ContentFolder.update(content_folder)
      ...
      {template, new_params} =
        case File.read(paths.absolute_path) do
          {:ok, body} ->
          ...
          _ ->
            ...
            error_params =
              case File.read(error_paths.absolute_path) do
                {:ok, error_body} ->
                  params
                  |> Map.put("path", "404.html")
                  |> Map.put("raw", "true")
                  |> Map.put("body", error_body)

                _ ->
                  params
                  |> Map.put("path", "404.html")
                  |> Map.put("raw", "true")
                  |> Map.put("body", "500 Internal Server Error")
              end
      ...
      Phoenix.View.render(BasicWeb.PageLiveView, template,
        params: new_params,
        ui: %{"css" => "communitex"},
        error_message: nil
      )
